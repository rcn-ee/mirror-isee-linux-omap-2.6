 ...
 */
static int mt9p031_set_config(struct v4l2_subdev *subdev, int irq, void *pdata)
{
...
static struct v4l2_subdev_file_ops mt9p031_subdev_file_ops = {
	.open = mt9p031_open,
	.close = mt9p031_close,
};
			 ...
			 const struct i2c_device_id *did)
{
	...
	mt9p031->pad.flags = MEDIA_PAD_FLAG_OUTPUT;
	...
	if (gpio_is_valid(pdata->reset)) {
		ret = gpio_request_one(pdata->reset, GPIOF_OUT_INIT_LOW,
					"mt9p031_rst");
		if (ret < 0) {
			dev_err(&client->dev, "MT9P031 could not request "
				"mt9p031_rst (%d)\n", ret);
			goto done;
		}
