...
#define		MT9V034_ROW_START_DEF			5 /* was 4 for SBGGR */
...
static int mt9v034_read(struct i2c_client *client, const u8 reg)
{
	...
	dev_dbg(&client->dev, "mt9v034:%s: read 0x%04x from 0x%02x\n", __func__, data, reg);
	return data;
}
...
static int mt9v034_update_aec_agc(struct mt9v034 *mt9v034, u16 which, int enable)
{
	struct i2c_client *client = v4l2_get_subdevdata(&mt9v034->subdev);
					 ...
					 unsigned int hratio)
{
	...
	ret = v4l2_ctrl_s_ctrl(mt9v034->pixel_rate,
...
static int mt9v034_set_config(struct v4l2_subdev *subdev, int irq, void *pdata)
{
	struct i2c_client *client = v4l2_get_subdevdata(subdev);
	struct mt9v034 *mt9v034 = to_mt9v034(subdev);
	s32 data;
...
static struct v4l2_subdev_core_ops mt9v034_subdev_core_ops = {
	...
	.s_config	= mt9v034_set_config,
};

static struct v4l2_subdev_file_ops mt9v034_subdev_file_ops = {
	.open = mt9v034_open,
	.close = mt9v034_close,
};
...
//	mt9v034->subdev.internal_ops = &mt9v034_subdev_internal_ops;
	...
	mt9v034->pad.flags = MEDIA_PAD_FLAG_OUTPUT;
